...
#[from_iterator(usize option)]
struct NewTypeAttrStruct(Vec<usize>);

#[derive(Default, FromIterator)]
struct NewTypeAttrField(#[from_iterator(usize option)] Vec<usize>);

#[derive(Default, FromIterator)]
struct MultiUnnamedAttrField(#[from_iterator(usize option)] Vec<usize>, bool);
    ...
    ($Struct:ident {instance . $expr:expr}) => {
        paste! {
            ...
            fn [<test_ $Struct:camel:lower _option>]() {
                ...
                let instance: $Struct = [Some(0), None, None, Some(1), None, Some(2), Some(3)].into_iter().collect();
                assert_eq!(instance.$expr, vec![0, 1, 2, 3]);
            }
        }
    };
